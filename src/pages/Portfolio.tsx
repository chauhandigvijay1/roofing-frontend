    ...
    <section className="pt-36 pb-24 px-6 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900">Our Work</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((p, i) => (
            <motion.div
              ...
              className="bg-white rounded-2xl overflow-hidden shadow-lg border border-slate-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
            >
              <img src={p.img} alt={p.title} className="w-full h-56 object-cover" />
              <div className="p-6">
                <h3 className="text-lg font-bold text-slate-900">{p.title}</h3>
                <p className="text-sm text-slate-500">{p.location}</p>
                <p className="text-xs text-slate-400 mt-1">A {name} project</p>
              </div>
            </motion.div>
          ))}
        </div>
